...
use crate::elgamal::{self, ElGamalCiphertext};
...
impl HybridCiphertext {
    ...
    // 2. elgamal ciphertext ← ElGamalEncrypt(pk, aes key)
    let elgamal_ciphertext =
        ElGamalCiphertext::encrypt(&aes_key, public_key);

    // 3. (nonce, aes ciphertext) ← AESEncrypt(aes key, m)
    let aes_ciphertext =
        AESCiphertext::encrypt_message(&aes_key, message)?;

    // 4. return (elgamal ciphertext, (nonce, aes ciphertext))
    Ok(HybridCiphertext {
        elgamal_ciphertext,
        aes_ciphertext,
    }) 
    ...
    pub fn decrypt(&self, private_key: &Scalar) -> Result<Vec<u8>, String> {
        // 1. aes key ← ElGamalDecrypt(sk, elgamal ciphertext)
    let aes_key = self.elgamal_ciphertext.decrypt(private_key);

    // 2. m ← AESDecrypt(aes key, (nonce, aes ciphertext))
    let plaintext =
