...
impl SchnorrSignature {
    ...
    // Compute pk = g^sk
    let pk = RISTRETTO_BASEPOINT_POINT * signing_key;

    // Hash(pk || R || message)
    let mut hasher = Sha512::new();
    hasher.update(SchnorrSignature::point_to_bytes(&pk));
    hasher.update(SchnorrSignature::point_to_bytes(&R));
    hasher.update(message);
    let hash_bytes = hasher.finalize();
    let h = Scalar::from_bytes_mod_order_wide(&hash_bytes.try_into().unwrap());
    ...
    pub fn verify(
        ...
        // Compute h = H(pk || R || m)
    let mut hasher = Sha512::new();
    hasher.update(SchnorrSignature::point_to_bytes(public_key));
    hasher.update(SchnorrSignature::point_to_bytes(&signature.R));
    hasher.update(message);
    let hash_bytes = hasher.finalize();
    let h = Scalar::from_bytes_mod_order_wide(&hash_bytes.try_into().unwrap());
    ...
    let right = signature.R + public_key * h;
