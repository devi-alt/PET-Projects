...
impl ElGamalCiphertext {
    ...
    pub fn encrypt(message: &Scalar, public_key: &RistrettoPoint) -> ElGamalCiphertext {
        ...
        hasher.update((r * public_key).compress().as_bytes());
        let hash_bytes = hasher.finalize();
        let hash_scalar = Scalar::from_bytes_mod_order_wide(&hash_bytes.try_into().unwrap());

        let c2 = message + hash_scalar;
    ...
    pub fn decrypt(&self, private_key: &Scalar) -> Scalar {
         // m = c2 - H(c1 * sk)
        let mut hasher = Sha512::new();
        hasher.update((self.c1 * private_key).compress().as_bytes());
        let hash_bytes = hasher.finalize();
        let hash_scalar = Scalar::from_bytes_mod_order_wide(&hash_bytes.try_into().unwrap());
